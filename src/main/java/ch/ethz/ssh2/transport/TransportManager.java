...
public class TransportManager {
				...
				if (useDisconnectPacket == true) {
					try {
						byte[] msg = new PacketDisconnect(
								Packets.SSH_DISCONNECT_BY_APPLICATION, cause
	...
	private void establishConnection(ProxyData proxyData, int connectTimeout)
		...
		if (proxyData instanceof HTTPProxyData) {
			...
			if ((pd.proxyUser != null) && (pd.proxyPass != null)) {
				...
				char[] encoded = Base64.encode(StringEncoder
	...
	public void initialize(CryptoWishList cwl, ServerHostKeyVerifier verifier,
		...
		tc = new TransportConnection(sock.getInputStream(), sock
				.getOutputStream(), rnd);
		...
		receiveThread = new Thread(new Runnable() {
			public void run() {
				...
				for (int i = 0; i < messageHandlers.size(); i++) {
					HandlerEntry he = (HandlerEntry) messageHandlers
							.elementAt(i);
					try {
